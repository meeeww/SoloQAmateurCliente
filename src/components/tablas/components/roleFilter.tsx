...
const RoleFilter: React.FC<RoleFilterProps> = ({ onFilterChange }) => {
  ...
  const roles = Object.entries(PlayerRole).map(([key, value]) => ({
    key,
    value,
    image: `https://api.koryubudoficial.es/assets/roleIcons/${value}.svg`,
  }));
      ...
      >
        {selectedRole ? (
          <div className="flex items-center">
            <img src={`https://api.koryubudoficial.es/assets/roleIcons/${selectedRole}.svg`} className="w-6 h-6 mr-2" alt={selectedRole} />
            {selectedRole}
          </div>
        ) : (
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 mt-1 rounded-lg bg-[#1f1f1f] shadow-lg z-10 border border-[#4a4a4a]">
          <div className="py-1" role="menu" aria-orientation="vertical">
            <div className="px-4 py-3 text-gray-300 hover:bg-[#2a2a2a] flex items-center cursor-pointer" onClick={() => handleRoleSelect("")}>
              Todos los roles
            </div>
            {roles.map((role) => (
              <div
